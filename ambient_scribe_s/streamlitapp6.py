...
<style>
    ...
    .block-container { max-width: 100%; padding: 0.5rem 1.5rem 0.25rem !important; }
    .stMarkdown, .stText, label, h1, h2, h3, p { color: var(--text-primary) !important; }
    .stMarkdown p { margin: 0 0 3px !important; }
    div[data-testid="stVerticalBlock"] > div { margin-top: 0 !important; }
    ...
    .toolbar-label { color: var(--text-muted) !important; font-size: 10px; font-weight: 750; letter-spacing: 1px; margin: 6px 0 2px; }
    .section-title { color: var(--text-primary); font-size: 16px; font-weight: 700; margin-top: 0; }
    .section-subtitle { color: var(--text-secondary); font-size: 11px; margin: 0 0 3px; }
    .section-gap { height: 4px; }
    .kpi-context { color: var(--text-muted); font-size: 10px; margin-top: 2px; }
    .patient-cell small, .insight-chip small { display: block; color: var(--text-secondary); font-size: 10px; }
    ...
    .patient-table-header-selected { grid-template-columns: 3.2fr 1.8fr 1.4fr 2fr; }
    .patient-cell { gap: 1px; padding-top: 2px; } .patient-cell b { font-size: 12px; } .table-meta { display: block; color: var(--text-secondary); font-size: 11px; padding-top: 4px; } .doctor-text { color: var(--info); }
    .table-row-rule { border-bottom: 1px solid var(--border); margin: 1px 0; } .pagination-label { color: var(--text-secondary); font-size: 11px; text-align: center; padding-top: 4px; }
    .clinical-graph { position: relative; overflow: hidden; margin: 5px 0 8px; background: #0F172A; border: 1px solid #1E293B; border-radius: 7px; }
    ...
    .insight-heading { color: var(--text-secondary); font-size: 10px; font-weight: 750; letter-spacing: .7px; text-transform: uppercase; margin: 6px 0 3px; }
    .workflow-steps { display: flex; gap: 0; margin: 8px 0 6px; color: var(--text-muted); font-size: 11px; } .workflow-steps span, .workflow-steps b { padding: 5px 14px; border-bottom: 2px solid var(--border); } .workflow-steps b { color: var(--primary); border-color: var(--primary); }
    .recording-prompt { color: var(--text-secondary); background: var(--surface-secondary); border: 1px dashed var(--border-strong); border-radius: 6px; padding: 10px; text-align: center; font-size: 12px; }
    .audit-note { color: var(--text-secondary); font-size: 11px; padding-bottom: 6px; }
    @media (max-width: 1100px) { .block-container { padding-left: 1rem !important; padding-right: 1rem !important; } }
...
def get_record_by_id(record_id):
    if not record_id:
        return None
    return next((record for record in st.session_state.records if record.get("id") == record_id), None)
...
def render_kpi_card(label, value, accent, context, on_click=None, button_key=None):
    if on_click:
        st.button(f"{label}\n{value}", key=button_key, on_click=on_click, help=context, use_container_width=True)
    else:
...
def render_patient_table(records, selected_date=None):
    ...
    if selected_date:
        st.markdown('<div class="patient-table-header patient-table-header-selected"><span>Patient &amp; Doctor</span><span>Date &amp; Time</span><span>Status</span><span>Actions</span></div>', unsafe_allow_html=True)
    else:
    ...
    for record in current_records:
        ...
        with row:
            if selected_date:
                cols = st.columns([3.2, 1.8, 1.4, 2.0])
                cols[0].markdown(f'<div class="patient-cell"><b>{escape(record.get("name", ""))}</b><small>{escape(record.get("id", ""))} · {escape(record.get("age", ""))}y {escape(record.get("gender", ""))} · {escape(record.get("doctor", "N/A"))}</small></div>', unsafe_allow_html=True)
                cols[1].markdown(f'<span class="table-meta">{escape(date_text)}<br>{escape(record.get("time", ""))}</span>', unsafe_allow_html=True)
                cols[2].markdown(render_status_badge(status), unsafe_allow_html=True)
                action_col = cols[3]
            else:
                ...
                action_col = cols[5]
            if action_col.button("Review" if status == "Pending" else "View", key=f"btn_{record.get('id', '')}", use_container_width=True):
                select_record(record.get("id"))
                st.rerun()
...
# ============================================================
def show_dashboard():
    render_header("Clinical Operations", "Consultation and documentation overview", show_new=False)

    # --- FILTERING & SEARCH ---
    st.markdown('<div class="toolbar-label">RECORD DIRECTORY</div>', unsafe_allow_html=True)
    filter_col, search_col, action_col = st.columns([3, 5, 2], gap="large")
    custom_dates = None
    with filter_col:
        date_filter = st.selectbox("Date Filter", ["All Time", "Today", "Last 7 Days", "Last 30 Days", "Custom Date Range"])
        status_filter = st.selectbox("Status", ["All Statuses", "Pending", "Approved"], key="status_filter")
    if date_filter == "Custom Date Range":
        with filter_col:
            custom_dates = st.date_input("Custom Range", value=(datetime.now().date(), datetime.now().date()))
    with search_col:
        search_query = st.text_input("Search", placeholder="Search patient or ID").strip().lower()
    with action_col:
        st.markdown('<div style="height: 24px;"></div>', unsafe_allow_html=True)
        if st.button("New Consultation", key="dashboard_new_consult", type="primary", use_container_width=True):
            st.session_state.live_chunks = []
            st.session_state.audio_key_counter = 0
            st.session_state.page = "new_consult"
            st.rerun()
    ...
    with k3: render_kpi_card("Pending Review", pending, "#B45309", "Requires attention", on_click=show_pending_records, button_key="pending_review_kpi")
    with k4: render_kpi_card("Approved Notes", approved, "#15803D", "Completed documentation")

    st.markdown('<div class="section-gap"></div>', unsafe_allow_html=True)

    # --- RENDER CHART AND PATIENT TABLE SIDE BY SIDE ---
    selected_date = st.session_state.chart_selected_date
    chart_col, table_col = st.columns([3, 7] if selected_date else [4.5, 5.5], gap="large")
    with chart_col:
        st.markdown('<div class="section-title">Daily Consultation Operations Snapshot</div><div class="section-subtitle">Daily consultation throughput and backlog</div>', unsafe_allow_html=True)
        if selected_date and st.button("Show all cases", key="clear_chart_filter", type="secondary"):
            st.session_state.chart_selected_date = None
    ...
    with table_col:
        st.markdown('<div class="section-title">Patient Records Directory</div><div class="section-subtitle">Clinical documentation queue</div>', unsafe_allow_html=True)
        if total > 0:
