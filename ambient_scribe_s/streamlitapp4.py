...
def show_dashboard():
    ...
    # --- RENDER TIME SERIES GRAPH (6-Hour Intervals) ---
    if total > 0:
        with st.container(border=True):
            st.markdown("<p style='font-weight:600; color:#1E293B;'>Consultation Activity (6-Hour Intervals)</p>", unsafe_allow_html=True)
            df = pd.DataFrame(filtered_records)

            # Ensure both date and time exist in the dataset
            if 'date' in df.columns and 'time' in df.columns:
                # 1. Combine Date and Time into a single string
                df['DatetimeStr'] = df['date'] + " " + df['time']

                # 2. Convert to a real DateTime object
                df['DatetimeObj'] = pd.to_datetime(df['DatetimeStr'], format="%d/%m/%Y %H:%M", errors='coerce')
                df_clean = df.dropna(subset=['DatetimeObj']).copy()

                if not df_clean.empty:
                    # 3. Floor the time to 6-hour buckets (00:00, 06:00, 12:00, 18:00)
                    df_clean['Interval'] = df_clean['DatetimeObj'].dt.floor('6h')

                    # Group by this new 6-hour interval
                    agg_df = df_clean.groupby(['Interval', 'status']).size().unstack(fill_value=0).reset_index()

                    if 'Pending' not in agg_df.columns: agg_df['Pending'] = 0
                    if 'Approved' not in agg_df.columns: agg_df['Approved'] = 0

                    agg_df['Total'] = agg_df['Pending'] + agg_df['Approved']

                    melted_df = agg_df.melt(id_vars=['Interval'], value_vars=['Total', 'Pending', 'Approved'], var_name='Metric', value_name='Count')

                    color_scale = alt.Scale(domain=['Total', 'Approved', 'Pending'], range=['#0D9488', '#10B981', '#F59E0B'])

                    # 4. Update the chart Axis to show the Time alongside the Date (e.g., 19/08 12:00)
                    line_chart = alt.Chart(melted_df).mark_line(point=True).encode(
                        x=alt.X('Interval:T', title='Date & Time', axis=alt.Axis(format="%d/%m %H:%M", labelAngle=-45, grid=False)),
                        ...
                        tooltip=[alt.Tooltip('Interval:T', format='%d/%m/%Y %H:%M', title='Interval Start'), 'Metric:N', 'Count:Q']
                    ).properties(height=280)

                    st.altair_chart(line_chart, use_container_width=True)
                else:
                    st.caption("Not enough valid temporal data to generate chart.")
    ...
    with st.container(border=True):
        ...
        if total > 0:
            ...
            if st.session_state.dash_page > total_pages: st.session_state.dash_page = 1
            ...
            for record in current_records:
                ...
                c1.markdown(f"<div style='color: #1E293B;'><b>{record.get('name','')}</b><br><span style='color: #64748B; font-size: 13px;'>{record.get('id','')} • {record.get('age','')}y {record.get('gender','')} • {record.get('doctor', 'N/A')}</span></div>", unsafe_allow_html=True)
                c2.markdown(f"<div style='color: #1E293B; margin-top: 5px;'>{record.get('date','')}<br><span style='color: #64748B; font-size: 13px;'>{record.get('time','')}</span></div>", unsafe_allow_html=True)
                status_color = "#F59E0B" if record.get("status") == "Pending" else "#10B981"
                c3.markdown(f"<div style='color: {status_color}; font-weight: bold; margin-top: 10px;'>{record.get('status','').upper()}</div>", unsafe_allow_html=True)

                with c4:
                    st.write("")
                    if st.button("Review Note" if record.get("status") == "Pending" else "View Record", key=f"btn_{record.get('id','')}", use_container_width=True):
