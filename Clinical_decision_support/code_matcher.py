...
def get_icd10_codes(
    ...
    items = normalize_detected_items(detected_conditions)
    fallback_results = []
    dataset_items = []
    for item in items:
        fallback = COMMON_ICD10_FALLBACKS.get(normalize_text(item))
        if fallback:
            fallback_results.append({
                "Extracted Condition": item,
                "Matched Disease/Injury": fallback["description"],
                "ICD-10 Code": fallback["code"],
            })
        else:
